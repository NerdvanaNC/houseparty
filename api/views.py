...
class JoinRoom(APIView):
  ...
  def post(self, request, format=None):
    ...
    return Response({'Bad Request': 'Invalid POST data; room not found'}, status=status.HTTP_400_BAD_REQUEST)
...
class CreateRoomView(APIView):
  ...
  def post(self, request, format=None):
    ...
    if serializer.is_valid():
      ...
      qs = Room.objects.filter(host=host)
      if qs.exists():
        room = qs[0]
  ...
  def patch(self, request, format=None):
    ...
    if serializer.is_valid():
      ...
      if room.host != user_id:
        return Response({'Message': 'Unauthorized Request. Not room host.'}, status=HTTP_403_FORBIDDEN)
